...
<div class="action  divaction">
	...
	<div class="pages">
		<div class="forms">
			<s:form action="claimVoucher_searchClaimVoucher.action"
				name="queryForm" method="get">
				<label>报销单状态</label>
				<s:select name="status" list="statusMap" listKey="key"
					listValue="value" headerKey="" headerValue="全部" theme="simple"></s:select>
		...
		<!--增加报销单 区域 开始-->
		<s:form action="" name="claimVoucherForm" method="post">
			<table width="90%" border="0" cellspacing="0" cellpadding="0"
				...
				<s:iterator value="pageSupport.items" id="claimVoucher" begin="0"
					status="s">
					<tr>
						<td><a
						...
						<td><s:property value="statusMap[#claimVoucher.status]" /></td>
						<td><s:property value="#claimVoucher.nextDeal.name" /></td>
						<td>
						<s:if test="#claimVoucher.nextDeal.sn == #session.employee.sn">
								<s:if
									test='#claimVoucher.status == "1" || #claimVoucher.status == "4"'>
									<a
						...
						<s:if
								test="#claimVoucher.nextDeal.sn == #session.employee.sn">
								<s:if
									test="#claimVoucher.status == \"2\" || #claimVoucher.status == \"3\" || #claimVoucher.status == \"5\"">
									<a
