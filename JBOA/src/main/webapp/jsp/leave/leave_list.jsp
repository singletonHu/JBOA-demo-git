...
<div class="action  divaction">
	...
	<div class="pages">
		<div class="forms">
		<s:form action="leave_searchLeave.action" name="queryForm" method="get">
	       ...
	       <label>审批状态</label>
           <s:select name="leave.status" list="statusMap" 
                listKey="key" listValue="value" headerKey="" headerValue="全部" theme="simple"></s:select>
           </s:if>
	...
	<!--增加报销单 区域 开始-->
		<table width="90%" border="0" cellspacing="0" cellpadding="0" class="list items">
	      ...
	      <tr>
	        ...
	        <td><s:date name="#leave.createTime" format="yyyy-MM-dd HH:mm:ss"/></td>
	        <td><s:date name="#leave.ModifyTime" format="yyyy-MM-dd HH:mm:ss"/></td>
	        <td><s:property value="#leave.approveOpinion"/></td>
	        <td><s:property value="statusMap[#leave.status]"/></td>
	        <td>
	       	 <a href="leave_getLeaveById.action?leave.id=<s:property value="#leave.id"/>"><img src="${images}/search.gif" width="16" height="15" /></a>
	       	  <s:if test="#leave.nextDeal.sn == #session.employee.sn">
		        <s:if test="#leave.status == \"1\"">
